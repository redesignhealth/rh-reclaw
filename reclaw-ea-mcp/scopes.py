"""Scope registry and enforcement helpers for reclaw-ea-mcp tools.

Adapted from reclaw-comms-mcp/scopes.py, itself adapted from rh-data-platform
``services/rh-mcp/scopes.py`` (TECH-2752), the canonical pattern named in the
RH tech guide (topics/04-auth-and-identity.md §Scope Enforcement).
...
``ScopeEnforcementMiddleware``, main.py).
...
claim -- this is the credential shape TECH-5084's agent run-loop host mints
per owner, per run.
"""
...
TOOL_SCOPES: dict[str, str] = {
    # --- ea (provider: providers/ea.py, namespace="ea") ---
    "ea_whoami": "ea:read",
    # Reads
    "ea_check_completion": "ea:read",
    # Writes / runs -- all mutate per-owner Negotiator state (negotiation
    # rounds, ledger holds, approval holds, outcome records)
    "ea_negotiate": "ea:write",
    "ea_react_to_conversation": "ea:run",
    "ea_request_booking": "ea:write",
    "ea_respond_to_approval": "ea:write",
}


# Resources gated for rh-auth callers (interactive Okta users bypass, like
# tools). Empty today -- this service registers no resources yet.
